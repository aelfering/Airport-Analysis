setwd("~/Documents/GitHub/Airport-Analysis/Cancellation Analysis/AlaskaAirportShiny")
...
ui <- shinyUI(fluidPage(  
    titlePanel("Percent of Flights Cancelled Among Major Alaskan Airports"),  
    sidebarLayout(  
        sidebarPanel(
            selectInput("airport", "Select an Airport:",
                        c('ANC', 'FAI', 'JNU', 'BET', 'KTN')),
            ...
            dateInput('date',
                      ...
                      max = max(as.Date(on_time_df$FL_DATE)),
                      value = max(as.Date(on_time_df$FL_DATE))),
            width=2),
        mainPanel(
...
server <- shinyServer(function(input, output) {  
    output$Plot <- renderPlot({  
        ...
        title_text <- paste(airport_name, ' Airport: ', can_rate, ' of Flights were Cancelled Between ', format(strptime(begin_day, format = "%Y-%m-%d"), "%b-%d"), ' and ', format(strptime(selection_date, format = "%Y-%m-%d"), "%b-%d"), sep = '')
